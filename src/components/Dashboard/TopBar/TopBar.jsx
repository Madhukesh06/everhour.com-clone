...
export default function TopBar({ handleTask }) {
  ...
  const user = JSON.parse(localStorage.getItem("everhourUser"));
  useEffect(() => {
    const getTasks = setTimeout(() => {
      let url = `https://everhour-backend-production.up.railway.app/users/tasks/query/${user.email}?q=${text}`;
    ...
    <>
      ...
      {text && (
        <Box
          ...
          py={7}
        >
          <Box px={12} pb={4} fontSize={14} fontWeight={600} color="gray.400">
            ACTIVE PROJECTS
          </Box>
          {data?.map((el) => (
            <>
